...
const RISK_ASSESSMENT = {
  ...
  self_harm: false,
  details: 'Denies SI/HI. No self-harm behaviors reported.',
};

async function main() {
  const client = createAdminClient();

  // Check if encounter for this date already exists
  ...
  if (existing) {
    console.log(`Encounter for ${ENCOUNTER_DATE} already exists (${existing.id}). Patching existing data...`);
  } else {
    // Create encounter
  ...
  // Patch risk_assessment on the canary note (idempotent — works for new and existing data)
  const { error: riskErr } = await client
    .from('clinical_notes')
    .update({ risk_assessment: RISK_ASSESSMENT })
    .eq('encounter_id', ENCOUNTER_ID)
    .eq('is_current', true);

  if (riskErr) throw new Error(`Failed to patch risk_assessment: ${riskErr.message}`);
  console.log('Patched risk_assessment on canary note');

  // Fix 2: Set low authorization — update sessions_used to 21 (of 24 authorized = 3 remaining)
  // This triggers the <=4 low-auth alert in the context loader
  const { error: insErr } = await client
    .from('patient_insurance')
    .update({ sessions_used: 21 })
    .eq('patient_id', PATIENT_ID)
    .eq('org_id', ORG_ID);

  if (insErr) throw new Error(`Failed to update sessions_used: ${insErr.message}`);
  console.log('Set sessions_used=21 on John Doe insurance (3 remaining of 24 → low-auth alert)');
  ...
  console.log('  5. Risk assessment (denies SI/HI/SH)');
  console.log('  6. Low authorization (3 sessions remaining)');
